...
namespace InventorySystem.Controllers
{
	public class AccountsReceivableController : Controller
	{
		...
		// GET: AccountsReceivable/CreditMemos
		public async Task<IActionResult> CreditMemos()
		{
			try
			{
				...
				var creditMemoList = adjustments.Select(a => new CreditMemoListViewModel
				{
					AdjustmentId = a.Id,
					...
					RelatedSaleNumber = a.Sale?.SaleNumber,
					RelatedSaleId = a.SaleId,
					Reason = a.Reason,
			...
			{
				_logger.LogError(ex, "Error loading credit memos");
				TempData["ErrorMessage"] = $"Error loading credit memos: {ex.Message}";
				return View(new List<CreditMemoListViewModel>());
			}
		}
	...
	public class CreditMemoListViewModel
	{
		...
		public string? RelatedSaleNumber { get; set; }
