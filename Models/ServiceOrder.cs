...
namespace InventorySystem.Models
{
  public class ServiceOrder
  {
    ...
    // ? NEW: Worksheet tracking fields
    [Display(Name = "Worksheet Required")]
    public bool WorksheetRequired { get; set; }

    [Display(Name = "Worksheet Uploaded")]
    public bool WorksheetUploaded { get; set; }

    [Display(Name = "Worksheet Uploaded Date")]
    public DateTime? WorksheetUploadedDate { get; set; }

    [Display(Name = "Worksheet Uploaded By")]
    [StringLength(100)]
    public string? WorksheetUploadedBy { get; set; }
    ...
    // ? NEW: Computed property to check if all required documents are uploaded
    [NotMapped]
    [Display(Name = "Required Documents Complete")]
    public bool RequiredDocumentsComplete
    {
      get
      {
        // Check certificate requirement
        if (CertificateRequired && !CertificateGenerated)
          return false;

        // Check worksheet requirement
        if (WorksheetRequired && !WorksheetUploaded)
          return false;
    ...
    // ? UPDATED: CanComplete now checks for required documents
    public bool CanComplete()
    {
      // Must be in the right status
      var statusOk = Status == ServiceOrderStatus.InProgress || Status == ServiceOrderStatus.QualityCheck;

      // All required documents must be uploaded
      var documentsOk = RequiredDocumentsComplete;

      return statusOk && documentsOk;
    }
    ...
    // ? NEW: Check if worksheet is required but not uploaded
    public bool RequiresWorksheet()
    {
      return WorksheetRequired && !WorksheetUploaded;
    }

    // ? NEW: Get list of missing required documents
    [NotMapped]
