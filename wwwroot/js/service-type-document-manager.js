// Service Type Document Management Module
class ServiceTypeDocumentManager {
    ...
    toggleDocumentUpload() {
        ...
        if (isVisible) {
            ...
            // FIXED: Check if form exists before trying to reset
            const form = document.getElementById('documentUploadForm');
            if (form) {
                form.reset();
            } else {
                console.warn('Document upload form not found for reset');
            }
        } else {
    ...
    async uploadDocument() {
        ...
        if (!form) {
            ...
            alert('Document upload form is not properly initialized. Please refresh the page.');
            return;
        }
        ...
        // FIXED: Match the exact parameter names expected by the controller
        formData.append('serviceTypeId', this.serviceTypeId);
        formData.append('file', file);
        
        // FIXED: Add null checks for form elements
        const documentTypeEl = document.getElementById('documentType');
        const documentNameEl = document.getElementById('documentName');
        const documentDescEl = document.getElementById('documentDescription');
        
        formData.append('documentType', documentTypeEl ? documentTypeEl.value || 'General' : 'General');
        formData.append('documentName', documentNameEl ? documentNameEl.value || '' : '');
        formData.append('description', documentDescEl ? documentDescEl.value || '' : '');
        ...
        try {
            console.log('Starting document upload...', {
                ...
                fileName: file.name,
                fileSize: file.size,
                documentType: documentTypeEl ? documentTypeEl.value : 'General',
                documentName: documentNameEl ? documentNameEl.value : '',
                description: documentDescEl ? documentDescEl.value : ''
            });

            const response = await fetch('/Services/UploadServiceTypeDocument', {
                method: 'POST',
                body: formData
            });
            ...
            if (data.success) {
                // FIXED: Only reset form if it exists
                if (form) {
                    form.reset();
                }
                ...
                console.error('Upload failed with server error:', data.message);
            ...
            let userMessage = 'Error uploading document: ';
            ...
            if (error.message.includes('HTTP 413')) {
                ...
                userMessage += 'Invalid request. Please check the file and try again.';
    ...
    async loadDocumentsList() {
        try {
            const response = await fetch(`/Services/GetServiceTypeDocuments?serviceTypeId=${this.serviceTypeId}`);
            const data = await response.json();
        ...
        return `
            <div class="d-flex justify-content-between align-items-center border-bottom py-2" id="document-${doc.id}">
                    ...
                    <div>
                        <h6 class="mb-1">
                            <a href="javascript:void(0)"
                               onclick="window.serviceTypeDocumentManager.viewDocument(${doc.id})"
                ...
                <div class="btn-group btn-group-sm">
                    <button class="btn btn-outline-primary" onclick="window.serviceTypeDocumentManager.downloadDocument(${doc.id})" title="Download">
                        <i class="fas fa-download"></i>
                    </button>
                    <button class="btn btn-outline-info" onclick="window.serviceTypeDocumentManager.viewDocument(${doc.id})" title="View">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="btn btn-outline-danger" onclick="window.serviceTypeDocumentManager.deleteDocument(${doc.id}, '${doc.documentName}')" title="Delete">
    ...
    getNoDocumentsHtml() {
        return `
            <div id="noDocumentsMessage" class="text-center py-4">
                ...
                <p class="text-muted">Upload specifications, procedures, training materials, or other relevant documents for this service type.</p>
                <button class="btn btn-outline-primary" onclick="window.serviceTypeDocumentManager.toggleDocumentUpload()">
    ...
    downloadDocument(documentId) {
        window.open(`/Services/DownloadServiceTypeDocument/${documentId}`, '_blank');
    }
    ...
    async viewDocument(documentId) {
        try {
            ...
            window.open(`/Services/ViewServiceTypeDocument/${documentId}`, '_blank');
        }
    }
    ...
    showDocumentInModal(documentId, documentInfo) {
        ...
        if (!modal || !modalTitle || !modalContent || !downloadBtn) {
            console.error('Modal elements not found, opening in new window');
            window.open(`/Services/ViewServiceTypeDocument/${documentId}`, '_blank');
            return;
        }
            ...
            modalContent.innerHTML = `
                <iframe src="/Services/ViewServiceTypeDocument/${documentId}"
                    ...
                    <p>Your browser doesn't support PDF viewing.
                       <a href="/Services/DownloadServiceTypeDocument/${documentId}" target="_blank">Download the PDF</a>
                    </p>
                </iframe>
            `;
    ...
    getUnsupportedDocumentHtml(documentId, documentInfo) {
        return `
            <div class="text-center p-4">
                ...
                <button class="btn btn-primary" onclick="window.serviceTypeDocumentManager.downloadDocument(${documentId})">
    ...
    async deleteDocument(documentId, documentName) {
        ...
        try {
            ...
            const response = await fetch('/Services/DeleteServiceTypeDocument', {
                method: 'POST',
                headers: {
        ...
        document.addEventListener('contextmenu', (e) => {
            if (e.target.closest('.document-name-link')) {
                ...
                contextMenu.innerHTML = `
                    <a class="dropdown-item" href="javascript:void(0)" onclick="window.serviceTypeDocumentManager.viewDocument(${documentId}); this.parentNode.remove();">
                        <i class="fas fa-eye"></i> Preview
                    </a>
                    <a class="dropdown-item" href="javascript:void(0)" onclick="window.serviceTypeDocumentManager.downloadDocument(${documentId}); this.parentNode.remove();">
                        <i class="fas fa-download"></i> Download
                    </a>
                    <div class="dropdown-divider"></div>
                    <a class="dropdown-item text-danger" href="javascript:void(0)" onclick="window.serviceTypeDocumentManager.deleteDocument(${documentId}, '${link.textContent}'); this.parentNode.remove();">
...
// Global functions for backward compatibility - these now check window.serviceTypeDocumentManager
window.toggleServiceTypeDocumentUpload = function() {
    if (window.serviceTypeDocumentManager && typeof window.serviceTypeDocumentManager.toggleDocumentUpload === 'function') {
        window.serviceTypeDocumentManager.toggleDocumentUpload();
    } else {
        console.error('serviceTypeDocumentManager not initialized');
    }
};

window.uploadServiceTypeDocument = function() {
    if (window.serviceTypeDocumentManager && typeof window.serviceTypeDocumentManager.uploadDocument === 'function') {
        window.serviceTypeDocumentManager.uploadDocument();
    } else {
        console.error('serviceTypeDocumentManager not initialized');
    }
};
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = ServiceTypeDocumentManager;
}